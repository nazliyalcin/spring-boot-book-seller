...
public class UserPrincipal implements UserDetails {
    ...
    public static UserPrincipal createSuperUser(){
        Set<GrantedAuthority> authorities;
         authorities = Set.of(SecurityUtils.convertToAuthority(Role.SYSTEM_MANAGER.name()));

         return UserPrincipal.builder().id(-1L).username("system-admin").authoritySet(authorities).build();
    }
    private Long id;
